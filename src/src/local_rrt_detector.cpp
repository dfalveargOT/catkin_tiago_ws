...
while (mapData.header.seq<1 or mapData.data.size()<1)  {ros::ROS_INFO("/* map not recieved localf*/");  ros::spinOnce();  ros::Duration(0.1).sleep();}
...
{
ros::spinOnce();
ros::ROS_INFO("/* points not recieved localf*/");
pub.publish(points) ;
}
